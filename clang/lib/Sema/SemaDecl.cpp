...
Decl *Sema::BuildAnonymousStructOrUnion(Scope *S, DeclSpec &DS,
  ...
  if (RecordDecl *OwningClass = dyn_cast<RecordDecl>(Owner)) {
    Anon = FieldDecl::Create(
        ...
        DeclarationName(), Context.getTypeDeclType(Record), TInfo,
...
Sema::ActOnFunctionDeclarator(Scope *S, Declarator &D, DeclContext *DC,
  ...
  if (getLangOpts().CPlusPlus) {
    ...
    if (const CXXRecordDecl *Parent =
      ...
      if (!LateMethodParameterInfoStack.empty() && D.isFunctionDefinition()) {
        LateParsedMethodParameterInfo &Info = LateMethodParameterInfoStack.back();
        if (!Info.ExpandedDecls.empty()) {
          LateMethodParameterInfo[NewFD] = Info;
        }
      }
...
void Sema::FinalizeDeclaration(Decl *ThisDecl) {
  ...
  for (const auto *I : ThisDecl->specific_attrs<TypeTagForDatatypeAttr>()) {
    ...
    if (!(MagicValueInt = MagicValueExpr->getIntegerConstantExpr(EvalCtx))) {
...
Decl *Sema::ActOnStartOfFunctionDef(Scope *FnBodyScope, Decl *D,
  ...
  auto IntroduceParameter = [this, FD, FnBodyScope](ParmVarDecl *Param) {
  ...
  // Introduce the parameters on the function declaration (which may
  // have been generated by a non-dependent parameter pack expansion).
  for (auto Param : FD->parameters())
    IntroduceParameter(Param);

  // Introduce any previously expanded parameter packs.
  auto LateMethodParamInfoEntry = LateMethodParameterInfo.find(FD);
  if (LateMethodParamInfoEntry != LateMethodParameterInfo.end()) {
    LateParsedMethodParameterInfo &Info = LateMethodParamInfoEntry->second;

    // Swap to the provided instantiation scope.
    //
    // This is reversed in Sema::ActOnFinishFunctionBody where we
    // additionally cleanup this LateParsedMethodParameterInfo object.
    assert(Info.OriginalScope == CurrentInstantiationScope);
    CurrentInstantiationScope = Info.Scope;

    for (auto *Param : Info.ExpandedDecls)
      IntroduceParameter(Param);
  }
...
Decl *Sema::ActOnFinishFunctionBody(Decl *dcl, Stmt *Body,
  ...
  if (FD) {
    ...
    auto ExpandedParameterPacks = LateMethodParameterInfo.find(FD);
    if (ExpandedParameterPacks != LateMethodParameterInfo.end()) {
      LateParsedMethodParameterInfo &Info = ExpandedParameterPacks->second;

      // Restore to the original scope, and drop this
      // LateParsedMethodParameterInfo object.
      LocalInstantiationScope::deleteScopes(CurrentInstantiationScope, Info.OriginalScope);
      LateMethodParameterInfo.erase(ExpandedParameterPacks);
    }
...
FieldDecl *Sema::HandleField(Scope *S, RecordDecl *Record,
  ...
  LookupResult Previous(*this, Name, Loc, LookupMemberName,
...
FieldDecl *Sema::CheckFieldDecl(DeclarationName Name, QualType T,
  ...
  if (BitWidth) {
    BitWidth = VerifyBitField(Loc, Name, T, Record->isMsStruct(Context),
                              BitWidth, &ZeroWidth).get();
    if (!BitWidth) {
      InvalidDecl = true;
      BitWidth = nullptr;
      ZeroWidth = false;

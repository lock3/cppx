...
void Sema::ActOnStartOfBlueLambdaDefinition(
  ...
  if (NoDeclarator) {
    ...
    clang::QualType ReturnType = TrailingReturn.isNull() ?
      DefaultTypeForNoTrailingReturn : TrailingReturn;
    QualType MethodTy = Context.getFunctionType(
      ReturnType, ParamTypes, EPI);
    MethodTyInfo =
    ...
    ExplicitResultType = TrailingReturn.isNull();
  ...
  for (auto C = Intro.Captures.begin(), E = Intro.Captures.end(); C != E;
    ...
    if (C->Init.isUsable()) {
      ...
      if (Var) {
        ...
        BlueSema.lookupUnqualifiedName(R);
        if (R.isAmbiguous())
          continue;
        if (R.empty()) {
        ...
        BlueSema.getCurrentScope()->Shadows.insert(Shadow);
      }
    } else {

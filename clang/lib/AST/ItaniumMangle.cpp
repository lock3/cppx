...
void CXXNameMangler::mangleValueInTemplateArg(QualType T, const APValue &V,
  ...
  case APValue::Fragment: {
    const auto *FE = cast<CXXFragmentExpr>(V.getFragmentExpr());

    Out << "ReF";
    Out << reinterpret_cast<std::uintmax_t>(FE);

    const APValue *FragCaps = V.getFragmentCaptures();
    for (unsigned I = 0; I < FE->getNumCaptures(); ++I) {
      mangleValueInTemplateArg(FE->getCapture(I)->getType(), FragCaps[I],
                               /*TopLevel=*/false, NeedExactType);
    }
    break;
  }

//===- BlueASTImporter.cpp - Custom AST importer --------------------------===//
//
...
using ExpectedName = llvm::Expected<DeclarationName>;
  ...
  class BlueASTNodeImporter : public TypeVisitor<BlueASTNodeImporter, ExpectedType>,
                          public DeclVisitor<BlueASTNodeImporter, ExpectedDecl>,
                          public StmtVisitor<BlueASTNodeImporter, ExpectedStmt> {
    blue::BlueASTImporter &Importer;
  ...
  public:
    explicit BlueASTNodeImporter(blue::BlueASTImporter &Importer) : Importer(Importer) {}

    using TypeVisitor<BlueASTNodeImporter, ExpectedType>::Visit;
    using DeclVisitor<BlueASTNodeImporter, ExpectedDecl>::Visit;
    using StmtVisitor<BlueASTNodeImporter, ExpectedStmt>::Visit;
...
template <typename InContainerTy>
Error BlueASTNodeImporter::ImportTemplateArgumentListInfo(
    SourceLocation FromLAngleLoc, SourceLocation FromRAngleLoc,
    const InContainerTy &Container, TemplateArgumentListInfo &Result) {
...
template <>
Error BlueASTNodeImporter::ImportTemplateArgumentListInfo<TemplateArgumentListInfo>(
...
Expected<BlueASTNodeImporter::FunctionTemplateAndArgsTy>
BlueASTNodeImporter::ImportFunctionTemplateWithTemplateArgsFromSpecialization(
    FunctionDecl *FromFD) {
...
BlueASTNodeImporter::import(TemplateParameterList *From) {
...
template <>
Expected<TemplateArgument>
BlueASTNodeImporter::import(const TemplateArgument &From) {
  switch (From.getKind()) {
...
BlueASTNodeImporter::import(const TemplateArgumentLoc &TALoc) {
...
template <>
Expected<DeclGroupRef> BlueASTNodeImporter::import(const DeclGroupRef &DG) {
  if (DG.isNull())
...
template <>
Expected<BlueASTNodeImporter::Designator>
BlueASTNodeImporter::import(const Designator &D) {
...
template <>
Expected<LambdaCapture> BlueASTNodeImporter::import(const LambdaCapture &From) {
  VarDecl *Var = nullptr;
...
template <typename T>
bool BlueASTNodeImporter::hasSameVisibilityContextAndLinkage(T *Found, T *From) {
...
template <>
bool BlueASTNodeImporter::hasSameVisibilityContextAndLinkage(TypedefNameDecl *Found,
...
ExpectedType BlueASTNodeImporter::VisitType(const Type *T) {
  Importer.FromDiag(SourceLocation(), diag::err_unsupported_ast_node)
    << T->getTypeClassName();
  return make_error<ImportError>(ImportError::UnsupportedConstruct);
}

ExpectedType BlueASTNodeImporter::VisitAtomicType(const AtomicType *T){
...
ExpectedType BlueASTNodeImporter::VisitBuiltinType(const BuiltinType *T) {
  switch (T->getKind()) {
...
ExpectedType BlueASTNodeImporter::VisitDecayedType(const DecayedType *T) {
...
ExpectedType BlueASTNodeImporter::VisitComplexType(const ComplexType *T) {
...
ExpectedType BlueASTNodeImporter::VisitPointerType(const PointerType *T) {
...
ExpectedType BlueASTNodeImporter::VisitBlockPointerType(const BlockPointerType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitLValueReferenceType(const LValueReferenceType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitRValueReferenceType(const RValueReferenceType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitMemberPointerType(const MemberPointerType *T) {
  // FIXME: Check for C++ support in "to" context.
  ExpectedType ToPointeeTypeOrErr = import(T->getPointeeType());
  if (!ToPointeeTypeOrErr)
    return ToPointeeTypeOrErr.takeError();
...
ExpectedType
BlueASTNodeImporter::VisitConstantArrayType(const ConstantArrayType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitIncompleteArrayType(const IncompleteArrayType *T) {
  ExpectedType ToElementTypeOrErr = import(T->getElementType());
  if (!ToElementTypeOrErr)
    return ToElementTypeOrErr.takeError();
...
ExpectedType
BlueASTNodeImporter::VisitVariableArrayType(const VariableArrayType *T) {
  Error Err = Error::success();
...
ExpectedType BlueASTNodeImporter::VisitDependentSizedArrayType(
...
ExpectedType BlueASTNodeImporter::VisitVectorType(const VectorType *T) {
  ExpectedType ToElementTypeOrErr = import(T->getElementType());
  if (!ToElementTypeOrErr)
    return ToElementTypeOrErr.takeError();
...
ExpectedType BlueASTNodeImporter::VisitExtVectorType(const ExtVectorType *T) {
  ExpectedType ToElementTypeOrErr = import(T->getElementType());
  if (!ToElementTypeOrErr)
    return ToElementTypeOrErr.takeError();
...
ExpectedType
BlueASTNodeImporter::VisitFunctionNoProtoType(const FunctionNoProtoType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitFunctionProtoType(const FunctionProtoType *T) {
  ExpectedType ToReturnTypeOrErr = import(T->getReturnType());
  if (!ToReturnTypeOrErr)
    return ToReturnTypeOrErr.takeError();
...
ExpectedType BlueASTNodeImporter::VisitUnresolvedUsingType(
...
ExpectedType BlueASTNodeImporter::VisitParenType(const ParenType *T) {
...
ExpectedType BlueASTNodeImporter::VisitTypedefType(const TypedefType *T) {
...
ExpectedType BlueASTNodeImporter::VisitTypeOfExprType(const TypeOfExprType *T) {
...
ExpectedType BlueASTNodeImporter::VisitTypeOfType(const TypeOfType *T) {
...
ExpectedType BlueASTNodeImporter::VisitDecltypeType(const DecltypeType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitUnaryTransformType(const UnaryTransformType *T) {
...
ExpectedType BlueASTNodeImporter::VisitAutoType(const AutoType *T) {
...
ExpectedType BlueASTNodeImporter::VisitInjectedClassNameType(
...
ExpectedType BlueASTNodeImporter::VisitRecordType(const RecordType *T) {
...
ExpectedType BlueASTNodeImporter::VisitEnumType(const EnumType *T) {
...
ExpectedType BlueASTNodeImporter::VisitAttributedType(const AttributedType *T) {
...
ExpectedType BlueASTNodeImporter::VisitTemplateTypeParmType(
...
ExpectedType BlueASTNodeImporter::VisitSubstTemplateTypeParmType(
    const SubstTemplateTypeParmType *T) {
...
ExpectedType BlueASTNodeImporter::VisitTemplateSpecializationType(
                                       const TemplateSpecializationType *T) {
...
ExpectedType BlueASTNodeImporter::VisitElaboratedType(const ElaboratedType *T) {
...
ExpectedType
BlueASTNodeImporter::VisitPackExpansionType(const PackExpansionType *T) {
...
ExpectedType BlueASTNodeImporter::VisitDependentTemplateSpecializationType(
    const DependentTemplateSpecializationType *T) {
  auto ToQualifierOrErr = import(T->getQualifier());
  if (!ToQualifierOrErr)
    return ToQualifierOrErr.takeError();
...
ExpectedType
BlueASTNodeImporter::VisitDependentNameType(const DependentNameType *T) {
  auto ToQualifierOrErr = import(T->getQualifier());
  if (!ToQualifierOrErr)
    return ToQualifierOrErr.takeError();
...
ExpectedType
BlueASTNodeImporter::VisitObjCInterfaceType(const ObjCInterfaceType *T) {
...
ExpectedType BlueASTNodeImporter::VisitObjCObjectType(const ObjCObjectType *T) {
  ExpectedType ToBaseTypeOrErr = import(T->getBaseType());
  if (!ToBaseTypeOrErr)
    return ToBaseTypeOrErr.takeError();
...
ExpectedType
BlueASTNodeImporter::VisitObjCObjectPointerType(const ObjCObjectPointerType *T) {
  ExpectedType ToPointeeTypeOrErr = import(T->getPointeeType());
  if (!ToPointeeTypeOrErr)
    return ToPointeeTypeOrErr.takeError();
...
// Import Declarations
//----------------------------------------------------------------------------
Error BlueASTNodeImporter::ImportDeclParts(
    NamedDecl *D, DeclContext *&DC, DeclContext *&LexicalDC,
    DeclarationName &Name, NamedDecl *&ToD, SourceLocation &Loc) {
  ...
  if (ToD)
    if (Error Err = BlueASTNodeImporter(*this).ImportDefinitionIfNeeded(D, ToD))
      return Err;

  return Error::success();
}

Error BlueASTNodeImporter::ImportDeclParts(NamedDecl *D, DeclarationName &Name,
...
Error BlueASTNodeImporter::ImportDefinitionIfNeeded(Decl *FromD, Decl *ToD) {
  if (!FromD)
    return Error::success();
...
Error
BlueASTNodeImporter::ImportDeclarationNameLoc(
    const DeclarationNameInfo &From, DeclarationNameInfo& To) {
...
Error
BlueASTNodeImporter::ImportDeclContext(DeclContext *FromDC, bool ForceImport) {
...
Error BlueASTNodeImporter::ImportDeclContext(
...
Error BlueASTNodeImporter::ImportImplicitMethods(
...
Error BlueASTNodeImporter::ImportDefinition(
...
Error BlueASTNodeImporter::ImportInitializer(VarDecl *From, VarDecl *To) {
...
Error BlueASTNodeImporter::ImportTemplateArguments(
    const TemplateArgument *FromArgs, unsigned NumFromArgs,
    SmallVectorImpl<TemplateArgument> &ToArgs) {
...
Expected<TemplateArgument>
BlueASTNodeImporter::ImportTemplateArgument(const TemplateArgument &From) {
  return import(From);
}
...
bool BlueASTNodeImporter::IsStructuralMatch(Decl *From, Decl *To, bool Complain) {
...
bool BlueASTNodeImporter::IsStructuralMatch(RecordDecl *FromRecord,
...
bool BlueASTNodeImporter::IsStructuralMatch(VarDecl *FromVar, VarDecl *ToVar,
                                        bool Complain) {
  StructuralEquivalenceContext Ctx(
      Importer.getFromContext(), Importer.getToContext(),
      Importer.getNonEquivalentDecls(), getStructuralEquivalenceKind(Importer),
...
bool BlueASTNodeImporter::IsStructuralMatch(EnumDecl *FromEnum, EnumDecl *ToEnum) {
  // Eliminate a potential failure point where we attempt to re-import
...
bool BlueASTNodeImporter::IsStructuralMatch(FunctionTemplateDecl *From,
                                        FunctionTemplateDecl *To) {
  StructuralEquivalenceContext Ctx(
      Importer.getFromContext(), Importer.getToContext(),
      Importer.getNonEquivalentDecls(), getStructuralEquivalenceKind(Importer),
...
bool BlueASTNodeImporter::IsStructuralMatch(FunctionDecl *From, FunctionDecl *To) {
  StructuralEquivalenceContext Ctx(
      Importer.getFromContext(), Importer.getToContext(),
      Importer.getNonEquivalentDecls(), getStructuralEquivalenceKind(Importer),
      false, false);
...
bool BlueASTNodeImporter::IsStructuralMatch(EnumConstantDecl *FromEC,
...
bool BlueASTNodeImporter::IsStructuralMatch(ClassTemplateDecl *From,
...
bool BlueASTNodeImporter::IsStructuralMatch(VarTemplateDecl *From,
                                        VarTemplateDecl *To) {
  StructuralEquivalenceContext Ctx(Importer.getFromContext(),
                                   Importer.getToContext(),
                                   Importer.getNonEquivalentDecls(),
...
ExpectedDecl BlueASTNodeImporter::VisitDecl(Decl *D) {
  Importer.FromDiag(D->getLocation(), diag::err_unsupported_ast_node)
    << D->getDeclKindName();
  return make_error<ImportError>(ImportError::UnsupportedConstruct);
}

ExpectedDecl BlueASTNodeImporter::VisitImportDecl(ImportDecl *D) {
  Importer.FromDiag(D->getLocation(), diag::err_unsupported_ast_node)
      << D->getDeclKindName();
  return make_error<ImportError>(ImportError::UnsupportedConstruct);
}

ExpectedDecl BlueASTNodeImporter::VisitEmptyDecl(EmptyDecl *D) {
  // Import the context of this declaration.
...
ExpectedDecl BlueASTNodeImporter::VisitTranslationUnitDecl(TranslationUnitDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitAccessSpecDecl(AccessSpecDecl *D) {
  ExpectedSLoc LocOrErr = import(D->getLocation());
  if (!LocOrErr)
    return LocOrErr.takeError();
...
ExpectedDecl BlueASTNodeImporter::VisitStaticAssertDecl(StaticAssertDecl *D) {
  auto DCOrErr = Importer.ImportContext(D->getDeclContext());
  if (!DCOrErr)
    return DCOrErr.takeError();
  DeclContext *DC = *DCOrErr;
...
ExpectedDecl BlueASTNodeImporter::VisitNamespaceDecl(NamespaceDecl *D) {
  ...
  if (!ToNamespace) {
    ...
    if (GetImportedOrCreateDecl(
            TempNS, D, Importer.getToContext(), DC, D->isInline(),
            *BeginLocOrErr, Loc, Name.getAsIdentifierInfo(),
            /*PrevDecl=*/nullptr, SC))
      return TempNS;
    ToNamespace = TempNS;
    TempNS->setRBraceLoc(*RBraceLocOrErr);
    TempNS->setLexicalDeclContext(LexicalDC);
...
ExpectedDecl BlueASTNodeImporter::VisitNamespaceAliasDecl(NamespaceAliasDecl *D) {
  // Import the major distinguishing characteristics of this namespace.
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
...
ExpectedDecl
BlueASTNodeImporter::VisitTypedefNameDecl(TypedefNameDecl *D, bool IsAlias) {
...
ExpectedDecl BlueASTNodeImporter::VisitTypedefDecl(TypedefDecl *D) {
  return VisitTypedefNameDecl(D, /*IsAlias=*/false);
}

ExpectedDecl BlueASTNodeImporter::VisitTypeAliasDecl(TypeAliasDecl *D) {
  return VisitTypedefNameDecl(D, /*IsAlias=*/true);
}

ExpectedDecl
BlueASTNodeImporter::VisitTypeAliasTemplateDecl(TypeAliasTemplateDecl *D) {
  // Import the major distinguishing characteristics of this typedef.
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
...
ExpectedDecl BlueASTNodeImporter::VisitLabelDecl(LabelDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitEnumDecl(EnumDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitRecordDecl(RecordDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitEnumConstantDecl(EnumConstantDecl *D) {
...
Error BlueASTNodeImporter::ImportTemplateParameterLists(const DeclaratorDecl *FromD,
                                                    DeclaratorDecl *ToD) {
...
Error BlueASTNodeImporter::ImportTemplateInformation(
...
BlueASTNodeImporter::FindFunctionTemplateSpecialization(FunctionDecl *FromFD) {
  auto FunctionAndArgsOrErr =
      ImportFunctionTemplateWithTemplateArgsFromSpecialization(FromFD);
  if (!FunctionAndArgsOrErr)
    return FunctionAndArgsOrErr.takeError();
...
Error BlueASTNodeImporter::ImportFunctionDeclBody(FunctionDecl *FromFD,
                                              FunctionDecl *ToFD) {
...
bool BlueASTNodeImporter::hasAutoReturnTypeDeclaredInside(FunctionDecl *D) {
...
ExplicitSpecifier
BlueASTNodeImporter::importExplicitSpecifier(Error &Err, ExplicitSpecifier ESpec) {
...
ExpectedDecl BlueASTNodeImporter::VisitFunctionDecl(FunctionDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitCXXMethodDecl(CXXMethodDecl *D) {
  return VisitFunctionDecl(D);
}

ExpectedDecl BlueASTNodeImporter::VisitCXXConstructorDecl(CXXConstructorDecl *D) {
  return VisitCXXMethodDecl(D);
}

ExpectedDecl BlueASTNodeImporter::VisitCXXDestructorDecl(CXXDestructorDecl *D) {
  return VisitCXXMethodDecl(D);
}

ExpectedDecl BlueASTNodeImporter::VisitCXXConversionDecl(CXXConversionDecl *D) {
  return VisitCXXMethodDecl(D);
}

ExpectedDecl
BlueASTNodeImporter::VisitCXXDeductionGuideDecl(CXXDeductionGuideDecl *D) {
  return VisitFunctionDecl(D);
}

ExpectedDecl BlueASTNodeImporter::VisitFieldDecl(FieldDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitIndirectFieldDecl(IndirectFieldDecl *D) {
  // Import the major distinguishing characteristics of a variable.
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
...
ExpectedDecl BlueASTNodeImporter::VisitFriendDecl(FriendDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitObjCIvarDecl(ObjCIvarDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitVarDecl(VarDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitImplicitParamDecl(ImplicitParamDecl *D) {
...
Error BlueASTNodeImporter::ImportDefaultArgOfParmVarDecl(
...
ExpectedDecl BlueASTNodeImporter::VisitParmVarDecl(ParmVarDecl *D) {
  // Parameters are created in the translation unit's context, then moved
  // into the function declaration's context afterward.
  DeclContext *DC = Importer.getToContext().getTranslationUnitDecl();
...
ExpectedDecl BlueASTNodeImporter::VisitObjCMethodDecl(ObjCMethodDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitObjCTypeParamDecl(ObjCTypeParamDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitObjCCategoryDecl(ObjCCategoryDecl *D) {
  // Import the major distinguishing characteristics of a category.
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
...
ExpectedDecl BlueASTNodeImporter::VisitObjCProtocolDecl(ObjCProtocolDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitLinkageSpecDecl(LinkageSpecDecl *D) {
  DeclContext *DC, *LexicalDC;
  if (Error Err = ImportDeclContext(D, DC, LexicalDC))
    return std::move(Err);
...
ExpectedDecl BlueASTNodeImporter::VisitUsingDecl(UsingDecl *D) {
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
  NamedDecl *ToD = nullptr;
...
ExpectedDecl BlueASTNodeImporter::VisitUsingShadowDecl(UsingShadowDecl *D) {
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
  NamedDecl *ToD = nullptr;
...
ExpectedDecl BlueASTNodeImporter::VisitUsingDirectiveDecl(UsingDirectiveDecl *D) {
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
  NamedDecl *ToD = nullptr;
...
ExpectedDecl BlueASTNodeImporter::VisitUnresolvedUsingValueDecl(
...
ExpectedDecl BlueASTNodeImporter::VisitUnresolvedUsingTypenameDecl(
...
ExpectedDecl BlueASTNodeImporter::VisitBuiltinTemplateDecl(BuiltinTemplateDecl *D) {
  Decl* ToD = nullptr;
...
Expected<ObjCTypeParamList *>
BlueASTNodeImporter::ImportObjCTypeParamList(ObjCTypeParamList *list) {
  if (!list)
    return nullptr;
...
ExpectedDecl BlueASTNodeImporter::VisitObjCInterfaceDecl(ObjCInterfaceDecl *D) {
...
ExpectedDecl
BlueASTNodeImporter::VisitObjCCategoryImplDecl(ObjCCategoryImplDecl *D) {
...
ExpectedDecl
BlueASTNodeImporter::VisitObjCImplementationDecl(ObjCImplementationDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitObjCPropertyDecl(ObjCPropertyDecl *D) {
...
ExpectedDecl
BlueASTNodeImporter::VisitObjCPropertyImplDecl(ObjCPropertyImplDecl *D) {
...
ExpectedDecl
BlueASTNodeImporter::VisitTemplateTypeParmDecl(TemplateTypeParmDecl *D) {
...
ExpectedDecl
BlueASTNodeImporter::VisitNonTypeTemplateParmDecl(NonTypeTemplateParmDecl *D) {
...
ExpectedDecl
BlueASTNodeImporter::VisitTemplateTemplateParmDecl(TemplateTemplateParmDecl *D) {
  // Import the name of this declaration.
...
ExpectedDecl BlueASTNodeImporter::VisitClassTemplateDecl(ClassTemplateDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitClassTemplateSpecializationDecl(
                                          ClassTemplateSpecializationDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitVarTemplateDecl(VarTemplateDecl *D) {
...
ExpectedDecl BlueASTNodeImporter::VisitVarTemplateSpecializationDecl(
...
ExpectedDecl
BlueASTNodeImporter::VisitFunctionTemplateDecl(FunctionTemplateDecl *D) {
  DeclContext *DC, *LexicalDC;
  DeclarationName Name;
  SourceLocation Loc;
  NamedDecl *ToD;
...
ExpectedStmt BlueASTNodeImporter::VisitStmt(Stmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitGCCAsmStmt(GCCAsmStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitDeclStmt(DeclStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitNullStmt(NullStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitCompoundStmt(CompoundStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitCaseStmt(CaseStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitDefaultStmt(DefaultStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitLabelStmt(LabelStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitAttributedStmt(AttributedStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitIfStmt(IfStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitSwitchStmt(SwitchStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitWhileStmt(WhileStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitDoStmt(DoStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitForStmt(ForStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitGotoStmt(GotoStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitIndirectGotoStmt(IndirectGotoStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitContinueStmt(ContinueStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitBreakStmt(BreakStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitReturnStmt(ReturnStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitCXXCatchStmt(CXXCatchStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitCXXTryStmt(CXXTryStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitCXXForRangeStmt(CXXForRangeStmt *S) {
...
ExpectedStmt
BlueASTNodeImporter::VisitObjCForCollectionStmt(ObjCForCollectionStmt *S) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitObjCAtCatchStmt(ObjCAtCatchStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitObjCAtFinallyStmt(ObjCAtFinallyStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitObjCAtTryStmt(ObjCAtTryStmt *S) {
...
ExpectedStmt
BlueASTNodeImporter::VisitObjCAtSynchronizedStmt(ObjCAtSynchronizedStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitObjCAtThrowStmt(ObjCAtThrowStmt *S) {
...
ExpectedStmt BlueASTNodeImporter::VisitObjCAutoreleasePoolStmt(
...
// Import Expressions
//----------------------------------------------------------------------------
ExpectedStmt BlueASTNodeImporter::VisitExpr(Expr *E) {
  Importer.FromDiag(E->getBeginLoc(), diag::err_unsupported_ast_node)
      << E->getStmtClassName();
  return make_error<ImportError>(ImportError::UnsupportedConstruct);
}

ExpectedStmt BlueASTNodeImporter::VisitVAArgExpr(VAArgExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitChooseExpr(ChooseExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitGNUNullExpr(GNUNullExpr *E) {
...
ExpectedStmt
BlueASTNodeImporter::VisitGenericSelectionExpr(GenericSelectionExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitPredefinedExpr(PredefinedExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitDeclRefExpr(DeclRefExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitImplicitValueInitExpr(ImplicitValueInitExpr *E) {
  ExpectedType TypeOrErr = import(E->getType());
  if (!TypeOrErr)
    return TypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitDesignatedInitExpr(DesignatedInitExpr *E) {
...
ExpectedStmt
BlueASTNodeImporter::VisitCXXNullPtrLiteralExpr(CXXNullPtrLiteralExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitIntegerLiteral(IntegerLiteral *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitFloatingLiteral(FloatingLiteral *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitImaginaryLiteral(ImaginaryLiteral *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitFixedPointLiteral(FixedPointLiteral *E) {
  auto ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitCharacterLiteral(CharacterLiteral *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitStringLiteral(StringLiteral *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitCompoundLiteralExpr(CompoundLiteralExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitAtomicExpr(AtomicExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitAddrLabelExpr(AddrLabelExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitConstantExpr(ConstantExpr *E) {
  Error Err = Error::success();
  auto ToSubExpr = importChecked(Err, E->getSubExpr());
...
ExpectedStmt BlueASTNodeImporter::VisitParenExpr(ParenExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitParenListExpr(ParenListExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitStmtExpr(StmtExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitUnaryOperator(UnaryOperator *E) {
  Error Err = Error::success();
  auto ToSubExpr = importChecked(Err, E->getSubExpr());
  auto ToType = importChecked(Err, E->getType());
...
BlueASTNodeImporter::VisitUnaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToOperatorLoc = importChecked(Err, E->getOperatorLoc());
  auto ToRParenLoc = importChecked(Err, E->getRParenLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitBinaryOperator(BinaryOperator *E) {
  Error Err = Error::success();
  auto ToLHS = importChecked(Err, E->getLHS());
  auto ToRHS = importChecked(Err, E->getRHS());
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitConditionalOperator(ConditionalOperator *E) {
  Error Err = Error::success();
  auto ToCond = importChecked(Err, E->getCond());
...
ExpectedStmt
BlueASTNodeImporter::VisitBinaryConditionalOperator(BinaryConditionalOperator *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitArrayTypeTraitExpr(ArrayTypeTraitExpr *E) {
  Error Err = Error::success();
  auto ToBeginLoc = importChecked(Err, E->getBeginLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitExpressionTraitExpr(ExpressionTraitExpr *E) {
  Error Err = Error::success();
  auto ToBeginLoc = importChecked(Err, E->getBeginLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitOpaqueValueExpr(OpaqueValueExpr *E) {
  Error Err = Error::success();
  auto ToLocation = importChecked(Err, E->getLocation());
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitArraySubscriptExpr(ArraySubscriptExpr *E) {
  Error Err = Error::success();
  auto ToLHS = importChecked(Err, E->getLHS());
  auto ToRHS = importChecked(Err, E->getRHS());
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt
BlueASTNodeImporter::VisitCompoundAssignOperator(CompoundAssignOperator *E) {
  Error Err = Error::success();
  auto ToLHS = importChecked(Err, E->getLHS());
  auto ToRHS = importChecked(Err, E->getRHS());
  auto ToType = importChecked(Err, E->getType());
...
BlueASTNodeImporter::ImportCastPath(CastExpr *CE) {
  CXXCastPath Path;
...
ExpectedStmt BlueASTNodeImporter::VisitImplicitCastExpr(ImplicitCastExpr *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitExplicitCastExpr(ExplicitCastExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToSubExpr = importChecked(Err, E->getSubExpr());
...
ExpectedStmt BlueASTNodeImporter::VisitOffsetOfExpr(OffsetOfExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitCXXNoexceptExpr(CXXNoexceptExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXThrowExpr(CXXThrowExpr *E) {
  Error Err = Error::success();
  auto ToSubExpr = importChecked(Err, E->getSubExpr());
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXDefaultArgExpr(CXXDefaultArgExpr *E) {
...
ExpectedStmt
BlueASTNodeImporter::VisitCXXScalarValueInitExpr(CXXScalarValueInitExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToTypeSourceInfo = importChecked(Err, E->getTypeSourceInfo());
  auto ToRParenLoc = importChecked(Err, E->getRParenLoc());
...
ExpectedStmt
BlueASTNodeImporter::VisitCXXBindTemporaryExpr(CXXBindTemporaryExpr *E) {
  ExpectedExpr ToSubExprOrErr = import(E->getSubExpr());
  if (!ToSubExprOrErr)
    return ToSubExprOrErr.takeError();
...
BlueASTNodeImporter::VisitCXXTemporaryObjectExpr(CXXTemporaryObjectExpr *E) {
  Error Err = Error::success();
...
ExpectedDecl BlueASTNodeImporter::VisitLifetimeExtendedTemporaryDecl(
...
ExpectedStmt
BlueASTNodeImporter::VisitMaterializeTemporaryExpr(MaterializeTemporaryExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitPackExpansionExpr(PackExpansionExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitSizeOfPackExpr(SizeOfPackExpr *E) {
  Error Err = Error::success();
  auto ToOperatorLoc = importChecked(Err, E->getOperatorLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXNewExpr(CXXNewExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXDeleteExpr(CXXDeleteExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToOperatorDelete = importChecked(Err, E->getOperatorDelete());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXConstructExpr(CXXConstructExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToLocation = importChecked(Err, E->getLocation());
  auto ToConstructor = importChecked(Err, E->getConstructor());
...
ExpectedStmt BlueASTNodeImporter::VisitExprWithCleanups(ExprWithCleanups *E) {
  ExpectedExpr ToSubExprOrErr = import(E->getSubExpr());
  if (!ToSubExprOrErr)
    return ToSubExprOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXMemberCallExpr(CXXMemberCallExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXThisExpr(CXXThisExpr *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXBoolLiteralExpr(CXXBoolLiteralExpr *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitMemberExpr(MemberExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt
BlueASTNodeImporter::VisitCXXPseudoDestructorExpr(CXXPseudoDestructorExpr *E) {
  Error Err = Error::success();
  auto ToBase = importChecked(Err, E->getBase());
  auto ToOperatorLoc = importChecked(Err, E->getOperatorLoc());
  auto ToQualifierLoc = importChecked(Err, E->getQualifierLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXDependentScopeMemberExpr(
...
ExpectedStmt
BlueASTNodeImporter::VisitDependentScopeDeclRefExpr(DependentScopeDeclRefExpr *E) {
  Error Err = Error::success();
  auto ToQualifierLoc = importChecked(Err, E->getQualifierLoc());
  auto ToTemplateKeywordLoc = importChecked(Err, E->getTemplateKeywordLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXUnresolvedConstructExpr(
...
ExpectedStmt
BlueASTNodeImporter::VisitUnresolvedLookupExpr(UnresolvedLookupExpr *E) {
...
ExpectedStmt
BlueASTNodeImporter::VisitUnresolvedMemberExpr(UnresolvedMemberExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToOperatorLoc = importChecked(Err, E->getOperatorLoc());
  auto ToQualifierLoc = importChecked(Err, E->getQualifierLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitCallExpr(CallExpr *E) {
  Error Err = Error::success();
  auto ToCallee = importChecked(Err, E->getCallee());
  auto ToType = importChecked(Err, E->getType());
  auto ToRParenLoc = importChecked(Err, E->getRParenLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitLambdaExpr(LambdaExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitInitListExpr(InitListExpr *E) {
  Error Err = Error::success();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXStdInitializerListExpr(
...
ExpectedStmt BlueASTNodeImporter::VisitCXXInheritedCtorInitExpr(
...
ExpectedStmt BlueASTNodeImporter::VisitArrayInitLoopExpr(ArrayInitLoopExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
...
ExpectedStmt BlueASTNodeImporter::VisitArrayInitIndexExpr(ArrayInitIndexExpr *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXDefaultInitExpr(CXXDefaultInitExpr *E) {
...
ExpectedStmt BlueASTNodeImporter::VisitCXXNamedCastExpr(CXXNamedCastExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToSubExpr = importChecked(Err, E->getSubExpr());
  auto ToTypeInfoAsWritten = importChecked(Err, E->getTypeInfoAsWritten());
...
ExpectedStmt BlueASTNodeImporter::VisitSubstNonTypeTemplateParmExpr(
...
ExpectedStmt BlueASTNodeImporter::VisitTypeTraitExpr(TypeTraitExpr *E) {
  Error Err = Error::success();
  auto ToType = importChecked(Err, E->getType());
  auto ToBeginLoc = importChecked(Err, E->getBeginLoc());
  auto ToEndLoc = importChecked(Err, E->getEndLoc());
...
ExpectedStmt BlueASTNodeImporter::VisitCXXTypeidExpr(CXXTypeidExpr *E) {
  ExpectedType ToTypeOrErr = import(E->getType());
  if (!ToTypeOrErr)
    return ToTypeOrErr.takeError();
...
ExpectedStmt BlueASTNodeImporter::VisitCXXFoldExpr(CXXFoldExpr *E) {
  Error Err = Error::success();
...
Error BlueASTNodeImporter::ImportOverriddenMethods(CXXMethodDecl *ToMethod,
                                               CXXMethodDecl *FromMethod) {
...
BlueASTImporter::BlueASTImporter(Sema& BlueSema, clang::ASTContext &ToContext,
    clang::FileManager &ToFileManager, clang::ASTContext &FromContext,
    clang::FileManager &FromFileManager, bool MinimalImport,
    std::shared_ptr<clang::ASTImporterSharedState> SharedState)
  :clang::ASTImporter(ToContext, ToFileManager, FromContext, FromFileManager,
...
llvm::Expected<clang::Decl *> BlueASTImporter::ImportImpl(clang::Decl *From) {
  // llvm_unreachable("Working on it.!");
  // Import the decl using BlueBlueASTNodeImporter.

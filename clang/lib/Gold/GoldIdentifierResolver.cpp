...
IdentifierResolver::IdentifierResolver(Sema &SemaRef)
  : IdDeclInfos(new IdDeclInfoMap), SemaRef(SemaRef) {}
...
bool IdentifierResolver::isDeclInScope(Declaration *D, DeclContext *Ctx, Scope *S,
                                       bool AllowInlineNamespace) const {
  Ctx = Ctx->getRedeclContext();

  if (Ctx->isFunctionOrMethod() || (S && S->isParameterScope())) {
    // Ignore the scopes associated within transparent declaration contexts.
    while (SemaRef.getDeclForScope(S) &&
           clang::Decl::castToDeclContext(SemaRef.getDeclForScope(S))->isTransparentContext())
      S = S->getParent();

    if (S->hasDeclaration(D->Op))
      return true;
    ...
    if (S->getParent()->isControlScope()) {
      ...
      if (S->getFlags() & Scope::FnTryCatchScope)
        return S->getParent()->hasDeclaration()(D->Op);
#endif
    }
  ...
  // that is its scope.
  DeclContext *DCtx = D->Cxx->getDeclContext()->getRedeclContext();
...
void IdentifierResolver::InsertDeclAfter(iterator Pos, Declaration *D) {
  DeclarationName Name(D->Id);
...
bool IdentifierResolver::tryAddTopLevelDecl(Declaration *D,
  ...
  if (isDeclPtr(Ptr)) {
    ...
    if (!PrevD->Cxx->getDeclContext()->getRedeclContext()->isTranslationUnit()) {
      IDI->AddDecl(D);
      IDI->AddDecl(PrevD);
    } else {
      IDI->AddDecl(PrevD);
  ...
  for (IdDeclInfo::DeclsTy::iterator I = IDI->decls_begin(),
    ...
    if (!(*I)->Cxx->getDeclContext()->getRedeclContext()->isTranslationUnit()) {
...
void IdentifierResolver::iterator::incrementSlowCase() {
  Declaration *D = **this;
  void *InfoPtr = DeclarationName(D->Id).getFETokenInfo();

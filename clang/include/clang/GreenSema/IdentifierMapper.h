...
#ifndef CLANG_GREEN_IDENTIFIER_TRANSFORM_H
#define CLANG_GREEN_IDENTIFIER_TRANSFORM_H
...
public:
  ...
  Decl *MapSyntax(const Syntax *S);

  Decl *MapMacro(const SyntaxMacro *S);
  Decl *MapCall(const SyntaxCall *S);
  Decl *MapIdentifier(const SyntaxIdent *S, QualType InTy);

  Decl *HandleOperatorEquals(const SyntaxMacro *S);

  QualType HandlePrefixColon(const SyntaxMacro *S);
...
private:
  ...
  llvm::PointerIntPair<StoredDeclsMap *, 1> CurrentSDM;

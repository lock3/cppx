//===- BlueSyntaxContext.h - Blues Syntactic context-----------------------===//
//
...
//===----------------------------------------------------------------------===//
//
//  Context to hold long-lived Syntax nodes.
//
//===----------------------------------------------------------------------===//

#ifndef CLANG_BLUE_BLUESYNTAXCONTEXT_H
#define CLANG_BLUE_BLUESYNTAXCONTEXT_H
...
/// like in clang::ASTContext. We want to keep the blue Syntax Tree and
/// C++ AST separate as they don't necessarily have the same lifetime, and
/// keeping them inside the C++ AST can confuse Clang.
inline void *operator new(size_t Bytes, const blue::SyntaxContext &C,
                          size_t Alignment /* = 8 */) {
  return C.Allocate(Bytes, Alignment);
}

inline void operator delete(void *Ptr, const blue::SyntaxContext &C, size_t) {
  C.Deallocate(Ptr);
}
...
inline void
operator delete[](void *Ptr, const blue::SyntaxContext &C, size_t) {
  C.Deallocate(Ptr);
}
#endif
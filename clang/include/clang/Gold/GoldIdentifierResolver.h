//===- GoldIdentifierResolver.h - Lexical Scope Name lookup ---------------===//
//
...
#ifndef CLANG_GOLD_IDENTIFIERRESOLVER_H
#define CLANG_GOLD_IDENTIFIERRESOLVER_H
...
class IdentifierResolver {
  ...
  class IdDeclInfo {
  public:
    using DeclsTy = llvm::SmallVector<Declaration *, 2>;
    ...
    void InsertDecl(DeclsTy::iterator Pos, Declaration *D) {
      Decls.insert(Pos, D);
    }
...
public:
  ...
  explicit IdentifierResolver(Sema &SemaRef);
  ~IdentifierResolver();

  /// begin - Returns an iterator for decls with the name 'Name'.
  iterator begin(clang::DeclarationName Name);
  ...
  ///        context.
  bool isDeclInScope(Declaration *D, clang::DeclContext *Ctx,
                     Scope *S = nullptr,
  ...
  /// position.
  void InsertDeclAfter(iterator Pos, Declaration *D);
  ...
  bool tryAddTopLevelDecl(Declaration *D, clang::DeclarationName Name);
